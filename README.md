...
```bash
DayPlannerApi$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
DayPlannerApi$ sam local start-api
DayPlannerApi$ curl http://localhost:3000/
```
...
```bash
DayPlannerApi$ sam logs -n NetCodeWebAPIServerless --stack-name DayPlannerApi --tail
```
...
```bash
DayPlannerApi$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name DayPlannerApi
```
